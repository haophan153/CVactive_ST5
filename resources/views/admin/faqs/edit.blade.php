...
<span class="text-gray-900 font-semibold truncate max-w-xs">{{ $faq->question }}</span>
@endsection
    ...
    <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
        <form action="{{ route('admin.faqs.update', $faq) }}" method="POST" class="space-y-5">
            ...
            <div>
                ...
                <input type="text" name="question" value="{{ old('question', $faq->question) }}" required class="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:ring-indigo-500 focus:border-indigo-500">
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Câu trả lời</label>
                <textarea name="answer" rows="6" required class="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm">{{ old('answer', $faq->answer) }}</textarea>
            </div>
            <div class="grid grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="number" name="sort_order" value="{{ old('sort_order', $faq->sort_order) }}" class="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm">
                </div>
            </div>
            <label class="flex items-center space-x-2 cursor-pointer">
                <input type="checkbox" name="is_active" value="1" {{ old('is_active', $faq->is_active) ? 'checked' : '' }} class="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500">
