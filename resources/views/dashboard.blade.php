                    ...
                    <div class="p-5">
                        <div class="flex items-center">
                            ...
                            <div class="ml-4">
                                ...
                                <p class="text-2xl font-bold text-gray-900">{{ $cvs->count() }}</p>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="p-5">
                        <div class="flex items-center">
                            ...
                            <div class="ml-4">
                                ...
                                <p class="text-2xl font-bold text-gray-900">{{ $cvs->where('is_draft', false)->count() }}</p>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="p-5">
                        <div class="flex items-center">
                            ...
                            <div class="ml-4">
                                ...
                                <p class="text-2xl font-bold text-gray-900">{{ $cvs->where('is_draft', true)->count() }}</p>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            @if($cvs->isEmpty())
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-12 text-center text-gray-500">
                        ...
                        <div class="mt-6">
                            <a href="{{ route('cv.create') }}" class="inline-flex items-center px-6 py-3 border border-transparent text-base font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 shadow-lg">
                                Tạo CV đầu tiên
                            </a>
                        </div>
                    </div>
                        ...
                        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg border border-gray-100 hover:shadow-lg transition-all duration-300 group">
                            ...
                            <div class="h-52 bg-gray-100 border-b relative overflow-hidden">
                                @if($cv->template && $cv->template->thumbnail)
                                    <img src="{{ $cv->template->thumbnail }}" alt="{{ $cv->title }}" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500">
                                @else
                                ...
                                <div class="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center space-x-3">
                                    <a href="{{ route('cv.edit', $cv) }}" class="bg-white text-gray-800 p-2.5 rounded-full hover:bg-indigo-600 hover:text-white transition-colors" title="Chỉnh sửa">
                                        <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                                        </svg>
                                    </a>
                                    <a href="{{ route('cv.pdf', $cv) }}" target="_blank" class="bg-white text-gray-800 p-2.5 rounded-full hover:bg-red-600 hover:text-white transition-colors" title="Tải PDF">
                                        <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
                                        </svg>
                                    </a>
                                    @if($cv->share_url ?? null)
                                    <a href="{{ $cv->share_url }}" target="_blank" class="bg-white text-gray-800 p-2.5 rounded-full hover:bg-blue-600 hover:text-white transition-colors" title="Xem link chia sẻ">
                                        <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    ...
                                    <div class="flex-1 min-w-0">
                                        ...
                                        <p class="text-sm text-gray-500 mt-1">
                                            <span class="inline-flex items-center">
                                                ...
                                                {{ $cv->template->name ?? 'Unknown' }}
                                            </span>
                                        </p>
                                        <p class="text-xs text-gray-400 mt-2">Cập nhật {{ $cv->updated_at->diffForHumans() }}</p>
                                    </div>
                                    
                                    {{-- Action Buttons --}}
                                    <div class="flex flex-col space-y-2 ml-3">
                                        <a href="{{ route('cv.edit', $cv) }}" class="bg-indigo-600 text-white hover:bg-indigo-700 px-4 py-2 rounded-lg text-sm font-medium text-center transition shadow-sm hover:shadow">
                                            Sửa
                                        </a>
                                        <div class="flex space-x-1">
                                            <a href="{{ route('cv.pdf', $cv) }}" target="_blank" class="flex-1 bg-gray-50 border border-gray-200 text-gray-600 hover:bg-gray-100 p-2 rounded-lg text-center transition" title="Tải PDF">
                                                <svg class="w-4 h-4 mx-auto" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
                                                </svg>
                                            </a>
                                            @if($cv->share_url ?? null)
                                            <a href="{{ $cv->share_url }}" target="_blank" class="flex-1 bg-gray-50 border border-gray-200 text-gray-600 hover:bg-blue-50 hover:border-blue-200 hover:text-blue-600 p-2 rounded-lg text-center transition" title="Xem link chia sẻ">
                                                <svg class="w-4 h-4 mx-auto" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            ...
                                            @endif
                                            <form action="{{ route('cv.destroy', $cv) }}" method="POST" class="flex-1">
                                                @csrf
                                                @method('DELETE')
