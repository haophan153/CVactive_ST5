...
@section('title', 'Việc làm - CVactive')
        ...
        <div class="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
            ...
            <div class="mt-8 flex flex-wrap justify-center gap-3">
                <a href="{{ route('jobs.index') }}" 
                   class="px-5 py-2 rounded-full text-sm font-medium transition-all duration-200 {{ !request('type') ? 'bg-white text-indigo-700 shadow-md' : 'bg-white/20 text-white hover:bg-white/30' }}">
                    Tất cả
                </a>
                <a href="{{ route('jobs.index', ['type' => 'full-time']) }}" 
                   class="px-5 py-2 rounded-full text-sm font-medium transition-all duration-200 {{ request('type') == 'full-time' ? 'bg-white text-indigo-700 shadow-md' : 'bg-white/20 text-white hover:bg-white/30' }}">
                    Toàn thời gian
                </a>
                <a href="{{ route('jobs.index', ['type' => 'part-time']) }}" 
                   class="px-5 py-2 rounded-full text-sm font-medium transition-all duration-200 {{ request('type') == 'part-time' ? 'bg-white text-indigo-700 shadow-md' : 'bg-white/20 text-white hover:bg-white/30' }}">
                    Bán thời gian
                </a>
                <a href="{{ route('jobs.index', ['type' => 'intern']) }}" 
                   class="px-5 py-2 rounded-full text-sm font-medium transition-all duration-200 {{ request('type') == 'intern' ? 'bg-white text-indigo-700 shadow-md' : 'bg-white/20 text-white hover:bg-white/30' }}">
                    Thực tập
                </a>
                <a href="{{ route('jobs.index', ['type' => 'contract']) }}" 
                   class="px-5 py-2 rounded-full text-sm font-medium transition-all duration-200 {{ request('type') == 'contract' ? 'bg-white text-indigo-700 shadow-md' : 'bg-white/20 text-white hover:bg-white/30' }}">
                    Hợp đồng
                </a>
            </div>
        </div>
    ...
    <div class="bg-white border-b shadow-sm">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
            <div class="flex justify-center gap-12">
                <div class="text-center">
                    <p class="text-2xl font-bold text-indigo-600">{{ $jobPosts->total() }}</p>
                    <p class="text-sm text-gray-500">Việc làm</p>
                </div>
                <div class="text-center">
                    <p class="text-2xl font-bold text-indigo-600">{{ $jobPosts->count() }}</p>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
            <div>
                ...
                @if(request('search') || request('location') || request('type'))
                    <p class="text-gray-500 mt-1">Tìm thấy {{ $jobPosts->total() }} việc làm phù hợp</p>
                @endif
            </div>
            
            @if(request('search') || request('location') || request('type'))
                <a href="{{ route('jobs.index') }}" class="inline-flex items-center gap-2 text-indigo-600 hover:text-indigo-800 font-medium">
        ...
        @if($jobPosts->count() > 0)
            ...
            <div class="grid gap-5">
                @foreach($jobPosts as $jobPost)
                    <a href="{{ route('jobs.show', $jobPost) }}" 
                        ...
                        <div class="flex items-start gap-5">
                            ...
                            <div class="flex-shrink-0">
                                @if($jobPost->company_logo)
                                    <img src="{{ asset('storage/' . $jobPost->company_logo) }}" 
                                ...
                                <div class="flex items-start justify-between gap-4">
                                    ...
                                    @if($jobPost->published_at)
                                        <span class="text-sm text-gray-400 whitespace-nowrap">
                                            {{ $jobPost->published_at->diffForHumans() }}
                                        </span>
                                    @endif
                                </div>
        ...
        @else
            ...
            <div class="text-center py-16">
                ...
                <a href="{{ route('jobs.index') }}" class="inline-flex items-center gap-2 px-6 py-3 bg-indigo-600 text-white rounded-xl hover:bg-indigo-700 transition">
                    <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
