        ...
        <div class="bg-white rounded-lg shadow p-6">
            ...
            <form method="POST" action="{{ route('hr.job-posts.update', $jobPost) }}" enctype="multipart/form-data">
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <textarea name="description" id="description" rows="6"
                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                            placeholder="Mô tả chi tiết về công việc, yêu cầu, quyền lợi..." required>{{ old('description', $jobPost->description) }}</textarea>
                        @error('description')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <select name="job_type" id="job_type" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                ...
                                <option value="intern" {{ old('job_type', $jobPost->job_type) == 'intern' ? 'selected' : '' }}>Thực tập</option>
                        ...
                        <div class="space-y-4">
                            ...
                            <div>
                                ...
                                <textarea name="company_description" id="company_description" rows="3"
                                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('company_description', $jobPost->company_description) }}</textarea>
                            </div>
                            <div>
                                <label for="company_logo" class="block text-sm font-medium text-gray-700">Logo công ty</label>
                                @if($jobPost->company_logo)
                                    <div class="mt-2 mb-2">
                                        <img src="{{ asset('storage/' . $jobPost->company_logo) }}" alt="Logo" class="w-20 h-20 object-contain">
                                    </div>
                                @endif
                                <input type="file" name="company_logo" id="company_logo" accept="image/*"
                                    class="mt-1 block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-semibold file:bg-indigo-50 file:text-indigo-700 hover:file:bg-indigo-100">
                                <p class="mt-1 text-xs text-gray-500">Để trống nếu muốn giữ nguyên logo cũ</p>
                            </div>
                        </div>
                    </div>
                ...
                <div class="mt-6 flex justify-end gap-4">
                    <a href="{{ route('hr.job-posts.show', $jobPost) }}" class="px-4 py-2 border border-gray-300 rounded-md text-gray-700 hover:bg-gray-50">
                        Hủy
                    </a>
