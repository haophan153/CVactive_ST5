...
<div class="py-6">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 lg:p-8">
            <h1 class="text-2xl font-bold text-gray-900 mb-6">Chỉnh sửa tin tuyển dụng</h1>

            <form method="POST" action="{{ route('hr.job-posts.update', $jobPost) }}" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="title" id="title" value="{{ old('title', $jobPost->title) }}"
                            class="mt-1 block w-full rounded-lg border-gray-200 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div>
                            ...
                            <select name="job_type" id="job_type" class="mt-1 block w-full rounded-lg border-gray-200 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                ...
                                <option value="full-time" {{ old('job_type', $jobPost->job_type) == 'full-time' ? 'selected' : '' }}>Toàn thời gian</option>
                                <option value="part-time" {{ old('job_type', $jobPost->job_type) == 'part-time' ? 'selected' : '' }}>Bán thời gian</option>
                    ...
                    <div>
                        ...
                        <textarea name="description" id="description" rows="7"
                            class="mt-1 block w-full rounded-lg border-gray-200 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>{{ old('description', $jobPost->description) }}</textarea>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div>
                                    ...
                                    @if($jobPost->company_logo)
                                        <div class="mt-1 mb-1">
                                            <img src="{{ $jobPost->company_logo_url }}" alt="Logo" class="w-12 h-12 object-contain rounded-lg border border-gray-100">
                                        </div>
                                    @endif
                                    ...
                                    <p class="mt-1 text-xs text-gray-400">Để trống nếu muốn giữ nguyên logo cũ</p>
                                </div>
                            </div>
                            <div>
                                ...
                                <textarea name="company_description" id="company_description" rows="3"
                                    class="mt-1 block w-full rounded-lg border-gray-200 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('company_description', $jobPost->company_description) }}</textarea>
                            </div>
                        </div>
                    </div>
                ...
                <div class="mt-8 flex justify-end gap-3">
                    <a href="{{ route('hr.job-posts.show', $jobPost) }}" class="px-5 py-2.5 border border-gray-200 rounded-lg text-gray-700 hover:bg-gray-50 font-medium text-sm transition">
                        Hủy
                    </a>
